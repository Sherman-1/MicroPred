...
class ProtT5Classifier(nn.Module):
    def __init__(self, base_model, num_classes, loss_weights=None):
        ...
        if loss_weights is not None: # Can't just rely on if lw : Boolean value of Tensor with more than one value is ambiguous, didn't know that !
...
def main():
    ...
    def data_collator(features):
        ...
        attention_mask = torch.stack([f[2] for f in features])
    ...
    training_args = TrainingArguments(
        output_dir='/store/EQUIPES/BIM/MEMBERS/simon.herman/MicroPred/models',
        num_train_epochs=3,
