...
def get_input_data():
    """
    Generates 50,000 random amino-acid sequences (10,000 per category),
    splits into train and eval sets, writes them to FASTA,
    and returns HF Datasets + class weights.
    """
    ...
    total_per_category = 50_000
    sequences, categories, ids = [], [], []
    ...
    for category in CLASSES:
        for i in range(total_per_category):
            ...
            spaced_seq = " ".join(seq)
            seq_id = f"{category}_{i}"
            
            sequences.append(spaced_seq)
    ...
    def save_fasta(df_subset, filepath):
        records = []
        for _, row in df_subset.iterrows():
            seq_nospaces = row["sequence"].replace(" ", "")
            rec = SeqRecord(Seq(seq_nospaces), id=row["seq_id"], description=row["category"])
            records.append(rec)
    ...
    save_fasta(train_df, "/store/EQUIPES/BIM/MEMBERS/simon.herman/MicroPred/random_classes_train_set.fasta")
    save_fasta(eval_df, "/store/EQUIPES/BIM/MEMBERS/simon.herman/MicroPred/random_classes_eval_set.fasta")
...
def main():
    ...
    train_ds.save_to_disk("/store/EQUIPES/BIM/MEMBERS/simon.herman/MicroPred/data/training_dataset/random_classes_training_dataset")
    eval_ds.save_to_disk("/store/EQUIPES/BIM/MEMBERS/simon.herman/MicroPred/data/training_dataset/random_classes_eval_dataset")
    ...
    training_args = TrainingArguments(
        ...
        output_dir="/store/EQUIPES/BIM/MEMBERS/simon.herman/MicroPred/models/random_classes_FT_MLP",
        num_train_epochs=3,
        ...
        save_steps=500,
        report_to=["wandb"],
        run_name="random_classes_FT_MLP"
    ...
    wandb.init(project="random_classes_FT_MLP", name="FT_DeepSpeed")
