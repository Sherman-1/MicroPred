...
USAGE = """
python HullRadV9.py [PDBcode].pdb
    or 
python HullRadV9.py [PDBcode].cif
"""
...
def Sved(all_atm_rec,num_MG,num_MN,model_array):
    ...
    return s,Dt,Dr,vbar_prot,Rht,ffo_hyd_P,prot_mol_mass,Ro,Rhr,int_vis,a_b_ratio, \
           Ft,Rg,Dmax,tauC,asphr,int_vis,ks,AA,NA,GL,DT,useNumpy
...
def compute_metrics(file):
    ...
    if extension == 'pdb' or extension == 'ent':
        result = model_from_pdb(file)
        if isinstance(result, int) and result == 1:
            
            return None 

        else:
            all_atm_rec,num_MG,num_MN,model_array = result

    elif extension == 'cif':

        result = model_from_cif(file)
        if isinstance(result, int) and result == 1:
    ...
    try:
        ...
        result = Sved(all_atm_rec,num_MG,num_MN,model_array)
        ...
        if isinstance(result, int) and result == 1:
            ...
            s,Dt,Dr,vbar_prot,Rht,ffo_hyd_P,prot_mol_mass,Ro,Rhr,int_vis,a_b_ratio,Ft,Rg,Dmax,tauC,asphr,int_vis,ks,AA,NA,GL,DT, useNumpy = result  
